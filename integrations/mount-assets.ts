    ...
    hooks: {
      "astro:build:done": ({ logger }) => {
        ...
        const meta: Record<string, Record<string, string>> = JSON.parse(
          fs.readFileSync("local.meta.json", "utf-8"),
        );
        ...
        fs.readdirSync("contents/posts", { withFileTypes: true }).forEach(
          (dir) => {
            ...
            // --- Check assets ---
            const assetsPath = `contents/posts/${dir.name}/assets`;
            try {
              if (!fs.statSync(assetsPath).isDirectory()) return;
            } catch {
              return;
            }

            // --- Mount assets ---
            const slug = meta.post[dir.name.slice(1) + ".md"];
        ...
        fs.readdirSync("contents/puzzles", { withFileTypes: true }).forEach(
          (dir): void => {
            ...
            // --- Check assets ---
            const assetsPath = `contents/puzzles/${dir.name}/assets`;
            try {
              if (!fs.statSync(assetsPath).isDirectory()) return;
            } catch {
              return;
            ...
            // --- Mount assets ---
            const slug = meta.puzzle[dir.name.slice(1) + ".md"];
